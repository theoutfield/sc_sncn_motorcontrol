...
// iEncoderSpeed = (60 * 1.000.000) / periode�sec
...
void run_hall( chanend c_hall, port in p_hall, port in p_encoder)
 ...
 //=========== encoder =======================
  ...
  iEncoderDividend = 75000 * iNrEncoderPulses;
...
//      p_ifm_ext_d2 <: 0;
	 ...
	 if(iEncoderStateOld != iEncoderPinState)
	 {
	     iEncoderStateOld = iEncoderPinState;
...
#define defPeriodMax 200000  //200msec
	  ...
	  if(iHallCalcFlag)
	  {
		  ...
		  {
		  iHallSpeed = iHallDividend / iHallPeriodMicroSeconds;  // period in �sec
		  }
...
//======================end HALL ===================================================================
  ...
  // cFlag ^= 0x01;     p_ifm_ext_d2 <: cFlag;  // test
   ...
   tx when timerafter(ts + 500) :> ts;   // 250 => 1�sec
	...
	#pragma ordered   // readout of 4 values about 700nsec
 	    select {
			case c_hall :> cmd:
			    if  (cmd == 1)
			    master
			    {   		c_hall <: iHallSpeed;   iHallSpeed &= 0x00FFFFFF;
			 	 	 	 	c_hall <: iHallAngle2;
