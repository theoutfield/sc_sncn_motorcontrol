		   	   	   	   	   	   	   	   	   	    	   	   	   	   ...
		   	   	   	   	   	   	   	   	   	    	   	   	   	   clock clk )
{
	...
	const unsigned adc_config_other[] = { 0b000000110000110000001111001100,   		/* Temperature */
		...
		iIndexADC &= 0x3;
		SPI_IDLE;
		t when timerafter(ts + delay_conv+ delay_acq) :> ts;
...
int xCount=100;
	...
	while (1)
	{
		...
		switch(xCount)
		{
		case 99:  if(iAdcNrReadings < 5){
			  	  iFilterSUM_Ia =  adc_data_a[4] * 256;
			  	  iFilterSUM_Ib =  adc_data_b[4] * 256;
		  	  	  }
		  	  	  if(iAdcNrReadings++ > 512) iAdcNrReadings=256;
		  	  	  break;
		case 98:  if(cmd==0)
		  	  	  {
			  	  iFilterSUM_Ia 	-= iCalibA;
			  	  iFilterSUM_Ia 	+= adc_data_a[4];
			  	  iCalibA            = iFilterSUM_Ia/256;
		  	  	  }
				  break;
		case 97:  if(cmd==0)
	  	  	  	  {
			  	  iFilterSUM_Ib 	-= iCalibB;
			  	  iFilterSUM_Ib 	+= adc_data_b[4];
			  	  iCalibB 	         = iFilterSUM_Ib/256;
	  	  	  	  }
				  break;
		case 96:  if(cmd == 1)
				  {
			 	  Ia = 	iCalibA  - adc_data_a[4];  // invert current direction
			 	  Ib = 	iCalibB  - adc_data_b[4];  // invert current direction
		          }
				  break;
		case 95:  if(cmd==0)
  	  	  	  	  {
...
#pragma ordered
	    select {
	    	case c_adc :> cmd:
				...
				c_adc <: adc_data_b[2];
				c_adc <: adc_data_b[3];
				xCount = 55;               // ===>> trigger for next adc conversion xx * 0,5�sec
				tx :> ts;
				}
	    	break;
		default:  break;
