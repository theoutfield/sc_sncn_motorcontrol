...
#define ENCODER_RESOLUTION          8192              // 4 x Max count of Incremental Encoder (4X decoding - quadrature mode)
...
/* Polarity is used to keep all position sensors to count ticks in the same direction
 *  (BISS_POLARITY_NORMAL/BISS_POLARITY_INVERTED) */
#define BISS_SENSOR_POLARITY        BISS_POLARITY_NORMAL
...
#define COMMUTATION_OFFSET_CLK      370//683
#define COMMUTATION_OFFSET_CCLK     3000//2731

/* Commutation offset (range 0-4095) (BISS sensor based commutation) */
#define COMMUTATION_OFFSET_BISS_CLK      2685//800
#define COMMUTATION_OFFSET_BISS_CCLK     585//1950
...
#endif
#if(SENSOR_USED == BISS)
    #define MAX_POSITION_LIMIT      GEAR_RATIO*ENCODER_RESOLUTION*2   // ticks (max range: 2^30, limited for safe operation)
    #define MIN_POSITION_LIMIT      -GEAR_RATIO*ENCODER_RESOLUTION*2   // ticks (min range: -2^30, limited for safe operation)
#endif
#if(SENSOR_USED == QEI)
