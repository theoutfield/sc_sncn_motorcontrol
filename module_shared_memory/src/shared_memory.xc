...
void shared_memory_service(server interface shared_memory_interface i_shared_memory[n], unsigned n){
    ...
    while (1) {
        ...
        case i_shared_memory[int j].write_angle_and_primary_feedback(unsigned int angle, unsigned int hall_state, int position, int velocity, SensorError sensor_error, unsigned int timestamp):
                data.angle = angle;
        ...
        case i_shared_memory[int j].write_angle_and_secondary_feedback(unsigned int angle, unsigned int hall_state, int position, int velocity, SensorError sensor_error, unsigned int timestamp):
                data.angle = angle;
                data.hall_state = hall_state;
                data.angle_velocity = velocity;
        ...
        case i_shared_memory[int j].write_primary_feedback(int position, int velocity, SensorError sensor_error, unsigned int timestamp):
                data.position = position;
                data.velocity = velocity;
                data.sensor_error = sensor_error;
                data.sensor_timestamp = timestamp;
        ...
        case i_shared_memory[int j].write_secondary_feedback(int position, int velocity, SensorError sensor_error, unsigned int timestamp):
                data.secondary_position = position;
                data.secondary_velocity = velocity;
                data.secondary_sensor_error = sensor_error;
                data.secondary_sensor_timestamp = timestamp;
