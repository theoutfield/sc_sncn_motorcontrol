...
void control_tuning_console(client interface MotionControlInterface i_motion_control)
{
    ...
    while (1)
    {
       ...
       case 'g':
                ...
                if(res_a >= 1)
                    printf("Open circuit fault in phase A\n");

                printf("Resistance in phase B = %.2f Om\n", res_b);
                if(res_b >= 1)
                    printf("Open circuit fault in phase B\n");

                printf("Resistance in phase C = %.2f Om\n", res_c);
                if(res_c >= 1)
                    printf("Open circuit fault in phase C\n");

                if(res_a < 1 && res_b < 1 && res_c < 1)
                    printf("Open circuit fault in phases not detected\n");
