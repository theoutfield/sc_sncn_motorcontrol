...
# Note: using abspath instead of relpath due to issues on Windows (see BUG 14285)
doc_dir = os.path.abspath(os.environ['DOC_DIR'])
user_theme_dir = os.path.abspath(os.path.join(doc_dir,"themes"))
...
def setup(app):
    ...
    if 'DATASHEET' in os.environ:
        xmos_datasheet_pdf = (os.environ['DATASHEET'] == "1")
    else:
        xmos_datasheet_pdf = False

    if xmos_datasheet_pdf:
        latex_doctype = 'datasheet'
        latex_section_numbers = True
        latex_section_newpage = True
        latex_toc = False
        use_sidecaption = False
    elif xmos_compact_pdf:
    ...
    for mod in extraconf_modules:
        ...
        if len(mod) > 0:
            try:
                ...
                extra_setup(app, conf, tags, globals())
                conf.set()
