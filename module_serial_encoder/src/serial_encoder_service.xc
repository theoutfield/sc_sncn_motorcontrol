...
void read_position(QEIHallPort * qei_hall_port_1, QEIHallPort * qei_hall_port_2, HallEncSelectPort * hall_enc_select_port, SPIPorts * spi_ports, port * biss_clock_port, port * biss_data_port, int hall_enc_select_config,
...
//        { state.position,state.status } = readRotarySensorAngleWithoutCompensation(*spi_ports, position_feedback_config.ifm_usec);
    ...
    case BISS_SENSOR:
        ...
        state.status = read_biss_sensor_data(qei_hall_port_1, qei_hall_port_2, hall_enc_select_port, hall_enc_select_config, biss_clock_port, biss_data_port, position_feedback_config.biss_config, data);
        t :> last_read;
        int count;
...
int init_sensor(QEIHallPort * qei_hall_port_1, QEIHallPort * qei_hall_port_2, HallEncSelectPort * hall_enc_select_port, SPIPorts * spi_ports, port * biss_clock_port, port * biss_data_port,
...
//        pos_state.status = initRotarySensor(*spi_ports,  position_feedback_config);
    ...
    for (int i=0;i<2;i++) { //read 2 times
        read_position(qei_hall_port_1, qei_hall_port_2, hall_enc_select_port, spi_ports, biss_clock_port, biss_data_port, hall_enc_select_config, position_feedback_config, sensor_type, pos_state, t, last_read);
    }
...
void serial_encoder_service(QEIHallPort * qei_hall_port_1, QEIHallPort * qei_hall_port_2, HallEncSelectPort * hall_enc_select_port, SPIPorts * spi_ports, port * movable (&?gpio_ports)[4],
                ...
                int gpio_on)
{
    ...
    // BiSS gpio port config
    port * movable biss_data_port;
    port * movable biss_clock_port;
    int biss_data_port_moved = 0, biss_clock_port_moved = 0;
    if (sensor_type == BISS_SENSOR) {
        if (position_feedback_config.biss_config.data_port_number >= ENCODER_PORT_EXT_D0) {
            biss_data_port = move(gpio_ports[position_feedback_config.biss_config.data_port_number - ENCODER_PORT_EXT_D0]);
            biss_data_port_moved = 1;
        }
        if (position_feedback_config.biss_config.clock_port_config <= BISS_CLOCK_PORT_EXT_D3) {
            biss_clock_port = move(gpio_ports[position_feedback_config.biss_config.clock_port_config & 0b11]);
            biss_clock_port_moved = 1;
        }
    }

    int read_period = init_sensor(qei_hall_port_1, qei_hall_port_2, hall_enc_select_port, spi_ports, biss_clock_port, biss_data_port, hall_enc_select_config, position_feedback_config, sensor_type, pos_state, t, last_read);
    ...
    while (loop_flag) {
        ...
        case i_position_feedback[int i].set_config(PositionFeedbackConfig in_config):
                ...
                read_period = init_sensor(qei_hall_port_1, qei_hall_port_2, hall_enc_select_port, spi_ports, biss_clock_port, biss_data_port, hall_enc_select_config, position_feedback_config, sensor_type, pos_state, t, last_read);
...
//                    { pos_state.last_position, pos_state.status } = readRotarySensorAngleWithoutCompensation(*spi_ports, position_feedback_config.ifm_usec);
        ...
        case i_position_feedback[int i].exit():
                if (biss_data_port_moved) {
                    gpio_ports[position_feedback_config.biss_config.data_port_number - ENCODER_PORT_EXT_D0] = move(biss_data_port);
                }
                if (biss_clock_port_moved) {
                    gpio_ports[position_feedback_config.biss_config.clock_port_config & 0b11] = move(biss_clock_port);
                }
                loop_flag = 0;
                continue;
