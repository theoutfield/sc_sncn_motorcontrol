...
void write_shared_memory(client interface shared_memory_interface ?i_shared_memory, SensorFunction sensor_function, int count, int position, int velocity, int angle, int hall_state, SensorError sensor_error, SensorError last_sensor_error, unsigned int timestamp)
{
    if (!isnull(i_shared_memory)) {
        ...
        case SENSOR_FUNCTION_COMMUTATION_AND_MOTION_CONTROL:
            i_shared_memory.write_angle_and_primary_feedback(angle, hall_state, count, position, velocity, sensor_error, last_sensor_error, timestamp);
            break;
        case SENSOR_FUNCTION_COMMUTATION_AND_FEEDBACK_DISPLAY_ONLY:
            i_shared_memory.write_angle_and_secondary_feedback(angle, hall_state, count, position, velocity, sensor_error, last_sensor_error, timestamp);
            break;
